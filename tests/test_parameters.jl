...
function whichtest(t::ASCIIString,net::HybridNetwork)
    ...
    elseif(t == "G")
        ...
        net.numht == [5,3,6,9] || error("net.numht wrong in case $(t)")
    elseif(t == "H")
        all(map(approxEq,net.ht,[0.1,1.0,1.0,1.0])) || error("net.ht wrong in case $(t)")
        net.numht == [3,3,5,7] || error("net.numht wrong in case $(t)")
    elseif(t == "J")
        all(map(approxEq,net.ht,[0.1,1.0,1.0,1.0])) || error("net.ht wrong in case $(t)")
        net.numht == [2,4,6,10] || error("net.numht wrong in case $(t)")
    elseif(t == "D")
    ...
    elseif(t == "I")
        ...
        net.numht == [3,3,6,10] || error("net.numht wrong in case $(t)")
    else
        error("not a known 5 taxon network case")
    end
    return tree
