...
## fixit: uncomment after ape version has evonet functions
...
net = readTopology(s);
# R"tree2 = read.tree(text = $s)" ## fixit: uncomment when CRAN has evonet functions
# @test convert(Bool, R"dist.topo($net, tree2, method='score') == 0")

# on a network, h=1, with useEdgeLength=false 
s = "(((A:1.0,(B:1.0)#H1:1.0::0.9):1.0,(C:1.0,#H1:1.0::0.1):1.0):1.0,D:1.0);";
net = readTopology(s);
net1 = apeRExport(net; useEdgeLength=false);
