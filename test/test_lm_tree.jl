...
## Fit a BM
fitBM = phyloNetworklm(trait ~ shift_m22 + shift_m57 + shift_m116, dat, phy)

# Tests against results obtained with geiger::fitContinuous or phylolm::phylolm
@test_approx_eq_eps loglikelihood(fitBM) -76.1541605207 1e-10 
@test_approx_eq_eps dof(fitBM)  5.0 1e-10
@test_approx_eq_eps aic(fitBM) 162.3083210414 1e-10 
@test_approx_eq_eps coef(fitBM) [4.8773804547 -0.6664678924 -1.0692950215 -0.0159349201] 1e-10 
...
@test_approx_eq_eps nobs(fitBM) 197.0 1e-10
@test_approx_eq_eps sum(residuals(fitBM)) -10.1752334428 1e-10
@test_approx_eq_eps dof_residual(fitBM) 193.0 1e-10
@test_approx_eq_eps sigma2_estim(fitBM) 0.0028624636 1e-10
@test_approx_eq_eps stderr(fitBM) [0.3413427535 0.5668153522 0.4972537835 0.3554989184] 1e-10
@test_approx_eq_eps confint(fitBM)[:,1] [4.2041393297 -1.7844157659 -2.0500444097 -0.7170966977] 1e-10
@test_approx_eq_eps confint(fitBM)[:,2] [5.5506215797 0.4514799811 -0.0885456333 0.6852268574] 1e-10
...
@test_approx_eq_eps predict(fitBM) predictR[fitBM.model.ind] 1e-10
