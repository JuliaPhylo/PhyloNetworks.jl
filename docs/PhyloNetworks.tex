...
\end{itemize}
These networks are
called \textit{level-1 networks} \citep{Huson2010}, and have been
shown to be identifiable \citep{Pardi2015,Solis-Lemus2015}.
...
To install the package, %\falta{(double check these steps)} 
open
...
\begin{lstlisting}
generalExport("examples/graph.dot")
\end{lstlisting}
This will turn
\texttt{graph.dot} into a file called \texttt{genImage.svg}
...
plotPhylonet(net, vert = false, mainTree = true, fontSize = 20.0)
\end{lstlisting}
