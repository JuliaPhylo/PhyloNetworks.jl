...
\item \textbf{readStartTop:} read a tree in parenthetical format from
  ...
  network yet. Both arguments are required. \textbf{Only branch lengths that
  are missing in the topology are updated.} \\  Usage:
\begin{lstlisting}
...
The function ends with ! because it modifies the DataCF object by
including the expected CF.
...
\end{lstlisting}
This function will modify the new dataframe by removing useless rows
such as \textit{sp1,sp1,sp1,sp1}, and averaging over repeated rows.
...
We can plot the observed CF to the expected CF of a given estimated
network to have a rough idea of the fitness to the data. If a network
is a good fit to the data, then the dots in the plot will be close to
the $y=x$ line.
\begin{itemize}
...
saved to PDF file (or many other formats based on the Gadfly tutorial):
\begin{lstlisting}
